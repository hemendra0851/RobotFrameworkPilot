	...
	def executeScripts(self):
		for i in self.RunManager_data.index:
			...
			if (self.RunManager_data['Run_Flag'][i]).lower() == 'yes':
				temp_dict['Suite_Name'] = str(self.RunManager_data['Test Suite Name'][i])
				testScriptPath = testAssetPath + "\\Scripts"
				self.mainoutputdir = testResultPath + "\\Result_" + timeStamp
				outputdir = self.mainoutputdir + "\\" + temp_dict['Suite_Name'][0:-6]
				self.outputXMLList = self.outputXMLList + outputdir + "\\output.xml "
				cmd = ("python -m robot --listener listener --console none --outputdir " 
					+ outputdir + " " + testScriptPath + "\\" + temp_dict['Suite_Name'])
				self.processCMD(cmd)
		return self.mainoutputdir

	def mergeResults(self):
		cmd = ("python -m robot.rebot --name Combined_TestSuite --logtitle Combined_Log "
			+ "--reporttitle Overall_Execution_Report --outputdir " + self.mainoutputdir 
			+ " " + self.outputXMLList)
		self.processCMD(cmd)
...
# 			temp_dict = {}
# 			if (self.RunManager_data['Run_Flag'][i]).lower() == 'yes':
# 				temp_dict['Suite_Name'] = str(self.RunManager_data['Test Case Name'][i])
# 				testScriptPath = testAssetPath + "\\Scripts\\"
# 				self.mainoutputdir = testResultPath + "\\Result_" + timeStamp
# 				suite = TestData(parent=None, source=testScriptPath + temp_dict['Suite_Name'])
# 				for testcase in suite.testcase_table:
# 					outputdir = self.mainoutputdir + "\\" + temp_dict['Suite_Name'][0:-6] + "\\" + testcase.name
# 					self.outputXMLList = self.outputXMLList + outputdir + "\\output.xml "
# 					cmd = ("python -m robot --listener listener --console none --outputdir " + outputdir 
# 					+ " -t " + testcase.name + " " + testScriptPath + temp_dict['Suite_Name'])
# 					self.processCMD(cmd)
# 		return self.mainoutputdir